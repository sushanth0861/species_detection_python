...
def pad_and_resize_cv(image_path, dataset, desired_size=32):
    ...
    clahe_img = apply_clahe_gray(img)
...
def transform_dataset(train_df, sample_df):
    ...
    for image_id in tqdm.tqdm(train_df['id'], desc='Processing train images'):
        train_resized_images.append(pad_and_resize_cv(image_id, 'train'))

    test_resized_images = []
    for image_id in tqdm.tqdm(sample_df['Id'], desc='Processing test images'):
        test_resized_images.append(pad_and_resize_cv(image_id, 'test'))
    ...
    transformed_data_directory = os.path.join(base_directory, 'transformed_data_clahe_gs')
    ...
    print("Saving content/transformed_data_clahe_gs/Resized_xTrain.npy")
    np.save(os.path.join(transformed_data_directory, 'Resized_yTrain.npy'), y_train)
    print("Saving content/transformed_data_clahe_gs/Resized_yTrain.npy")
    np.save(os.path.join(transformed_data_directory, 'Resized_xTest.npy'), X_test)
    print("Saving content/transformed_data_clahe_gs/Resized_xTest.npy")
